...
class InternetDesaController extends Controller
{
        ...
        try {
            ...
            $data = [
                ...
                'unit' => [
                    ...
                    'type' => $unit->type ?? 'Internet Desa'
                ],
                // TAMBAHAN: Variabel individual untuk kompatibilitas template
                'unitName' => $unit->unit_name ?? $unit->name ?? 'Internet Desa',
                'unitId' => $unit->id_units ?? $unit->id,
                'unitType' => $unit->type ?? 'Internet Desa',
                'summary' => $summary,
            ...
            try {
                ...
                $filename = "Detail-Transaksi-Intenet Desa-{$year}-{$month}.pdf";
    ...
    public function downloadExcelDetail($bulan)
    {
        try {
            ...
            $filename = "Detail-Transaksi-Internet Desa-{$year}-{$month}.xlsx";
    ...
    public function show($bulan)
    {
        ...
        return Inertia::render('kepala_bumdes/DetailLaporanInterdesa', [
            'auth' => [
                'user' => Auth::user()->only(['name', 'roles', 'image']),
            ],
