...
class DashboardKepalaBumdesController extends Controller
{
    ...
    public function index()
    {
        ...
        // Hardcode untuk unit 4 dan 5 saja
        $unitId = self::TARGET_UNIT_IDS;

        $dashboardData = $this->getDashboardData($unitId);

        return Inertia::render('kepala_bumdes/DashboardKepalaBumdes', [
            'unit_id' => $unitId,
            'auth' => [
    ...
    private function getDashboardData($unitId)
    {
        ...
        // Pendapatan bulan ini dari unit 4 dan 5 saja
        ...
        // Pengeluaran bulan ini dari unit 4 dan 5 saja
    ...
    private function getUnitBalances($unitIds)
    {
        ...
        // Handle jika unit 4 atau 5 tidak memiliki balance history
    ...
    public function updateSaldoAwal(Request $request)
    {
        ...
        // Validasi hanya untuk unit 4 dan 5
        if (!in_array($validated['id_unit'], self::TARGET_UNIT_IDS)) {
            return back()->withErrors(['error' => 'Akses ditolak. Hanya unit 4 dan 5 yang diizinkan.']);
        }
    ...
    private function getStatistics($unitId)
    {
        ...
        // Pendapatan bulan ini dan bulan lalu dari unit 4 dan 5 saja
        ...
        // Pengeluaran bulan ini dan bulan lalu dari unit 4 dan 5 saja
        $pengeluaranBulanIni = Expense::whereIn('unit_id', $unitId)
            ->where('created_at', '>=', $thisMonth)
            ->sum('nominal');
