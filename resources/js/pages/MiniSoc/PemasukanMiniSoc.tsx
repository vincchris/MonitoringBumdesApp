            ...
            <div className="flex items-center justify-between px-6 pt-6 pb-8 text-black">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-right">
                        ...
                        <p className="mr-3 text-xs text-gray-500">{user.roles}</p>
                    </div>
                </div>
            </div>
            ...
            {showModal && (
                <div className="bg-opacity-30 fixed inset-0 z-50 flex items-center justify-center backdrop-blur-[4px]">
                    <div className="relative w-full max-w-md rounded-xl bg-white p-6 text-black shadow-lg">
                        ...
                        <h2 className="mb-4 text-lg font-semibold">Tambah Pendapatan Mini Soccer</h2>
                    ...
                    <tbody>
                        {pemasukan.map((item, i) => (
                            <tr key={i} className="border-t">
                                ...
                                <td className="px-4 py-3">{item.tipe_penyewa}</td>
                                <td className="px-4 py-3">Rp. {item.tarif_per_jam.toLocaleString('id-ID')}</td>
                                <td className="px-4 py-3">Rp. {item.total_bayar.toLocaleString('id-ID')}</td>
                                <td className="px-4 py-3">{item.keterangan}</td>

                                {/* Kolom aksi */}
                                <td className="px-4 py-3">
                                    <div className="flex gap-2">
                                        <button type="button" className="rounded bg-yellow-500 px-4 py-2 text-white hover:bg-yellow-600" title="Edit">
                                            <Pencil size={16} />
                                        </button>
                                        <button type="button" className="rounded bg-red-600 px-4 py-2 text-white hover:bg-red-700" title="Hapus">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </td>
