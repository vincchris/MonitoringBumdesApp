...
const TarifCard: React.FC<{
    ...
    <StatsCard title="Tarif Sewa" icon={Calendar} gradient="bg-gradient-to-br from-purple-50 to-purple-100" iconColor="text-purple-600">
        <p className="mt-2 text-lg font-semibold text-gray-900">{tarif ? `${formatRupiah(tarif.harga_per_unit)}/bulan` : 'Per Kegiatan'}</p>
...
const TransactionRow: React.FC<{
    ...
    const handleDetailClick = useCallback(() => {
        router.visit(route('InterDesa.show', item.bulan));
    }, [item.bulan]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 p-4 backdrop-blur-md">
            <div className="max-h-[80vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-xl font-semibold text-gray-900">Daftar Tarif sewa Internet desa</h3>
...
export default function InterDesa() {
    ...
    const confirmDeleteTarif = useCallback(() => {
        ...
        router.delete(route('interdesa.deleteTarif', { unitId: DEFAULT_UNIT_ID, tarifId: deletingTarif.id_tarif }), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleSimpanSaldoAwal = useCallback(() => {
        ...
        router.post(
            route('interdesa.storeInitialBalance', DEFAULT_UNIT_ID),
            {
    ...
    const handleSimpanTarif = useCallback(() => {
        ...
        if (tarifMode === 'edit' && editingTarifId) {
            ...
            router.put(route('interdesa.updateTarif', { unitId: DEFAULT_UNIT_ID, tarifId: editingTarifId }), payload, {
                preserveScroll: true,
                onSuccess: () => {
            ...
            router.post(route('interdesa.storeTarif', { unitId: DEFAULT_UNIT_ID }), payload, {
                preserveScroll: true,
                onSuccess: () => {
                    setShowModalTarif(false);
                    resetTarifForm();
    ...
    return (
        <AppLayout>
            <Head title="Unit Usaha - Internet Desa" />
            ...
            <div className="min-h-screen bg-gray-50 px-4 py-6 sm:px-6 lg:px-8">
                ...
                <header className="mb-8">
                    ...
                    <p className="mt-2 text-sm text-gray-600">Kelola keuangan dan operasional Internet desa</p>
                </header>
            ...
            {/* Modal: Saldo */}
            <Modal isOpen={showModalSaldo} onClose={() => closeModal('saldo')} title="Atur Saldo Awal Internet desa">
            ...
            <Modal
                ...
                title={`${tarifMode === 'edit' ? 'Edit' : 'Tambah'} Tarif Sewa Internet desa`}
            >
                <div className="space-y-4">
                    <div>
                    ...
                    <div>
                        <label className="mb-1 block text-sm font-medium text-gray-700">Tarif per bulan</label>
                        <input
                            type="text"
                            ...
                            required
                        />
                        <p className="mt-1 text-xs text-gray-500">Masukkan tarif dalam rupiah per bulan</p>
                    </div>
                </div>
            ...
            <ConfirmationModal
                ...
                title="Hapus Tarif"
                message={`Apakah Anda yakin ingin menghapus tarif "${deletingTarif?.category_name}" dengan harga ${deletingTarif ? formatRupiah(deletingTarif.harga_per_unit) : ''}/bulan? Tindakan ini tidak dapat dibatalkan.`}
                confirmText="Hapus"
                cancelText="Batal"
                isDestructive={true}
            />
