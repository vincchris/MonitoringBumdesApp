...
const TarifCard: React.FC<{
    ...
    <StatsCard title="Tarif Sewa" icon={Calendar} gradient="bg-gradient-to-br from-purple-50 to-purple-100" iconColor="text-purple-600">
        <div className="mt-2">
            <p className="text-lg font-semibold text-gray-900">{tarif ? `${formatRupiah(tarif.harga_per_unit)}/Kegiatan` : 'Per Kegiatan'}</p>
        </div>
...
const TransactionRow: React.FC<{
    ...
    const handleDetailClick = useCallback(() => {
        router.visit(route('Buper.show', item.bulan));
    }, [item.bulan]);
...
// Modal Lihat Tarif dengan tombol edit dan delete per row
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 p-4 backdrop-blur-md">
            <div className="max-h-[80vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-lg font-semibold text-gray-900">Daftar Tarif Bumi Perkemahan</h3>
                                    ...
                                    data.map((item, index) => (
                                        <tr key={item.id_tarif} className="hover:bg-gray-50">
                                                ...
                                                <div className="flex items-center">
                                                    <span
                                                        className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${
                                                            item.category_name === '>300' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800'
                                                        }`}
                                                    >
                                                        {item.category_name}
                                                    </span>
                                            ...
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-gray-900">
                                                Per {item.satuan === 'kegiatan' ? 'Kegiatan' : item.satuan}
                                            </td>
                                            <td className="px-6 py-4 text-sm whitespace-nowrap">
                ...
                <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
                    <div className="rounded-lg bg-blue-50 p-4">
                        ...
                        <p className="mt-2 text-2xl font-bold text-blue-900">{data.length}</p>
                    </div>
                    ...
                    <div className="rounded-lg bg-green-50 p-4">
                        ...
                        <p className="mt-2 text-2xl font-bold text-green-900">
                            {data.length > 0 ? formatRupiah(Math.max(...data.map((item) => item.harga_per_unit))) : 'Rp 0'}
                        </p>
                    </div>
                    ...
                    <div className="rounded-lg bg-purple-50 p-4">
                        ...
                        <p className="mt-2 text-sm font-bold text-purple-900">
                            {data.length > 0
                                ? dayjs(
                                      data.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime())[0].updated_at,
                                  ).format('DD/MM/YYYY')
                                : '-'}
                        </p>
                    </div>
...
const formatHargaPerUnit = (value: string | number): string => {
...
export default function Buper() {
    ...
    // Handler untuk tombol "Atur" (create new tarif)
    const handleAturTarif = useCallback(() => {
        // Reset form untuk create baru
        setFormData({
            tanggal: dayjs().format('YYYY-MM-DD'), // Set tanggal hari ini sebagai default
            category_name: '',
            nominalBaru: '',
            jumlahMin: '1',
            jumlahMax: '300',
    ...
    const handleEditTarifFromView = useCallback((tarifData?: TarifDetail) => {
        if (tarifData) {
            // Mode edit dengan data specific
            setFormData({
                tanggal: dayjs().format('YYYY-MM-DD'), // Set tanggal hari ini untuk update
                category_name: tarifData.category_name || '',
                nominalBaru: '',
                jumlahMin: tarifData.category_name === '>300' ? '301' : '1',
                jumlahMax: tarifData.category_name === '>300' ? '999' : '300',
                hargaPerUnit: tarifData.harga_per_unit ? formatHargaPerUnit(tarifData.harga_per_unit) : '',
            });
            setTarifMode('edit');
            setEditingTarifId(tarifData.id_tarif);
        } else {
            // Mode create baru dari tombol "Tambah Tarif Baru"
            setFormData({
    ...
    const confirmDeleteTarif = useCallback(() => {
        ...
        router.delete(route('buper.deleteTarif', { unitId: DEFAULT_UNIT_ID, tarifId: deletingTarif.id_tarif }), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    // Handler untuk simpan/update tarif
    const handleSimpanTarif = useCallback(() => {
        const hargaPerUnit = parseCurrency(formData.hargaPerUnit);
        const min = parseInt(formData.jumlahMin);
        const max = parseInt(formData.jumlahMax);
        ...
        if (tarifMode === 'edit' && editingTarifId) {
            ...
            router.put(route('buper.updateTarif', { unitId: DEFAULT_UNIT_ID, tarifId: editingTarifId }), payload, {
                preserveScroll: true,
                onSuccess: () => {
            ...
            router.post(route('buper.storeTarif', { unitId: DEFAULT_UNIT_ID }), payload, {
                preserveScroll: true,
                onSuccess: () => {
                    setShowModalTarif(false);
                    resetTarifForm();
    ...
    }, [formData.tanggal, formData.hargaPerUnit, formData.jumlahMin, formData.jumlahMax, tarifMode, editingTarifId]);
    ...
    return (
        <AppLayout>
            <Head title="Unit Usaha - Bumi Perkemahan" />
            ...
            <div className="min-h-screen bg-gray-50 px-4 py-6 sm:px-6 lg:px-8">
                ...
                <header className="mb-8">
                    <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">Unit Usaha - Buper</h1>
            ...
            {/* Modal: Saldo */}
            <Modal isOpen={showModalSaldo} onClose={() => closeModal('saldo')} title="Atur Saldo Awal Buper">
                ...
                <div className="mt-6 flex justify-end gap-3">
                    <Button variant="outline" onClick={() => closeModal('saldo')}>
                        Batal
                    </Button>
            ...
            {/* Modal: Tarif Sewa */}
            <Modal isOpen={showModalTarif} onClose={() => closeModal('tarif')} title={`${tarifMode === 'edit' ? 'Edit' : 'Tambah'} Tarif Sewa Buper`}>
                <div className="space-y-4">
                    <div>
                    ...
                    {/* Show calculated category */}
                    <div className="rounded-lg bg-blue-50 p-3">
                        <label className="mb-1 block text-sm font-medium text-blue-900">Kategori yang akan disimpan:</label>
                        <span
                            className={`inline-flex items-center rounded-full px-3 py-1 text-sm font-medium ${
                                (parseInt(formData.jumlahMax) || 300) > 300 ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800'
                            }`}
                        >
                            {(parseInt(formData.jumlahMax) || 300) > 300 ? '>300' : '<=300'}
                        </span>
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            required
                        />
                        <p className="mt-1 text-xs text-gray-500">Masukkan tarif dalam rupiah per kegiatan</p>
                    </div>
                </div>
            ...
            <ConfirmationModal
                ...
                title="Hapus Tarif"
                message={`Apakah Anda yakin ingin menghapus tarif "${deletingTarif?.category_name}" dengan harga ${deletingTarif ? formatRupiah(deletingTarif.harga_per_unit) : ''}/kegiatan? Tindakan ini tidak dapat dibatalkan.`}
                confirmText="Hapus"
                cancelText="Batal"
                isDestructive={true}
            />
