...
const TarifCard: React.FC<{
    ...
    <StatsCard title="Tarif Sewa" icon={Calendar} gradient="bg-gradient-to-br from-purple-50 to-purple-100" iconColor="text-purple-600">
        <p className="mt-2 text-lg font-semibold text-gray-900">{tarif ? `${formatRupiah(tarif.harga_per_unit)}/m3` : 'Per Kegiatan'}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/10 p-4 backdrop-blur-md">
            <div className="max-h-[80vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-xl">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h3 className="text-xl font-semibold text-gray-900">Daftar Tarif sewa Air weslik</h3>
...
export default function airweslik() {
    ...
    const confirmDeleteTarif = useCallback(() => {
        ...
        router.delete(route('airweslik.deleteTarif', { unitId: DEFAULT_UNIT_ID, tarifId: deletingTarif.id_tarif }), {
            preserveScroll: true,
            onSuccess: () => {
    ...
    const handleSimpanSaldoAwal = useCallback(() => {
        ...
        router.post(
            route('airweslik.storeInitialBalance', DEFAULT_UNIT_ID),
            {
    ...
    const handleSimpanTarif = useCallback(() => {
        ...
        if (tarifMode === 'edit' && editingTarifId) {
            ...
            router.put(route('airweslik.updateTarif', { unitId: DEFAULT_UNIT_ID, tarifId: editingTarifId }), payload, {
                preserveScroll: true,
                onSuccess: () => {
            ...
            router.post(route('airweslik.storeTarif', { unitId: DEFAULT_UNIT_ID }), payload, {
                preserveScroll: true,
                onSuccess: () => {
                    setShowModalTarif(false);
                    resetTarifForm();
    ...
    return (
        <AppLayout>
            <Head title="Unit Usaha - Air Weslik" />
            ...
            <div className="min-h-screen bg-gray-50 px-4 py-6 sm:px-6 lg:px-8">
                ...
                <header className="mb-8">
                    <h1 className="text-2xl font-bold text-gray-900 sm:text-3xl">Unit Usaha - Air Weslik</h1>
                    <p className="mt-2 text-sm text-gray-600">Kelola keuangan dan operasional Air weslik</p>
                </header>
            ...
            {/* Modal: Saldo */}
            <Modal isOpen={showModalSaldo} onClose={() => closeModal('saldo')} title="Atur Saldo Awal Air weslik">
            ...
            <Modal
                ...
                title={`${tarifMode === 'edit' ? 'Edit' : 'Tambah'} Tarif Sewa air weslik`}
            >
                <div className="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="Contoh: Fasilitas umum, perumahan, perusahaan, dll"
                            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm text-black focus:border-blue-500 focus:ring-1 focus:ring-blue-500 focus:outline-none"
                        />
                    </div>

                    <div>
                        <label className="mb-1 block text-sm font-medium text-gray-700">Tarif per m3</label>
                        <input
                            type="text"
                            ...
                            required
                        />
                        <p className="mt-1 text-xs text-gray-500">Masukkan tarif dalam rupiah per m3</p>
                    </div>
                </div>
            ...
            <ConfirmationModal
                ...
                title="Hapus Tarif"
                message={`Apakah Anda yakin ingin menghapus tarif "${deletingTarif?.category_name}" dengan harga ${deletingTarif ? formatRupiah(deletingTarif.harga_per_unit) : ''}/m3? Tindakan ini tidak dapat dibatalkan.`}
                confirmText="Hapus"
                cancelText="Batal"
                isDestructive={true}
            />
